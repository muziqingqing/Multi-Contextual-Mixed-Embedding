...
void *glove_thread(void *vid) {
    ...
    FILE *fin;
    fin = fopen64(input_file, "rb");
    //off64_t num_lines=450867933;
    fseeko64(fin, (num_lines / num_threads * id) * (sizeof(CREC)), SEEK_SET); //Threads spaced roughly equally throughout file
    cost[id] = 0;
      // printf("cost %lld\n",id); 
...
int save_params() {
    ...
    if(use_binary > 0) { // Save parameters in binary file
        ...
        if(save_gradsq > 0) {
            ...
            fgs = fopen64(output_file_gsq,"wb");
...
int train_glove() {
    ...
    int b;
    FILE *fin;
    //off64_t h=4294967296;
   // off64_t m=4294967296/16;
    real total_cost = 0;
    fprintf(stderr, "TRAINING MODEL\n");
   fprintf(stderr,"%d\n",sizeof(off64_t));
    fin = fopen64(input_file, "rb");
    if(fin == NULL) {fprintf(stderr,"Unable to open cooccurrence file %s.\n",input_file); return 1;}
    fseeko64(fin, (off64_t)0LL, SEEK_END);
  // fseeko(fin, (off64_t)0LL, SEEK_END);
    file_size = ftello64(fin);
  //  file_size =h;
    fprintf(stderr,"file_size is %lld\n",file_size);
    num_lines = file_size/15; // Assuming the file isn't corrupt and consists only of CREC's
   ...
   // num_lines = m;
    ...
    for(b = 0; b < num_iter; b++) {
      ...
      // fprintf(stderr,"total_cost is  %lf\n",total_cost);
...
int main(int argc, char **argv) {
    ...
    else strcpy(input_file, (char *)"s.shuf.bin");
